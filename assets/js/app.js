...
// ---- Modal Areas de Atuação (robusto) ----
(function(){
  const ready = (fn)=>{
  ...
  ready(()=>{
    const MAP = AREA_TEXT || {};
    // Delegate clicks para garantir funcionamento mesmo após mudanças no DOM
    document.addEventListener('click', (ev)=>{
      const btn = ev.target.closest('.saiba');
      if(!btn) return;
      const card = btn.closest('.card');
      const titleEl = card ? card.querySelector('.card-body h3') : null;
      const key = btn.dataset.area;
      const title = titleEl ? titleEl.textContent.trim() : 'Área';
      const text = MAP[key] || '';
      const modal = document.getElementById('modal');
      if(!modal) return;
      modal.querySelector('#modal-title').textContent = title;
      modal.querySelector('#modal-content').innerHTML = '<p>'+text+'</p>';
